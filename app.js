...
function loadURLParams() {
  ...
  // Sync editor view once URL parameters are loaded
  syncEditorToState();
  updateCouplePhotoDisplay();
...
function showWelcomeModal() {
  ...
  // Show only on first visit (using wdp_welcomed localStorage flag)
  const welcomed = localStorage.getItem('wdp_welcomed');
  if (!welcomed) {
    modal.classList.remove('hidden');
    
    // Automatically close after 3.5 seconds (3500ms)
    setTimeout(() => {
      closeWelcomeModal();
    }, 3500);
  } else {
...
function closeWelcomeModal() {
  ...
  // Audio playback trigger on closing the welcome modal
  playAudioIfPossible();
}
